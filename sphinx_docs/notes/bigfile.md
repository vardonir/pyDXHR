...
- the PC version of the base game stores the game data in `BIGFILE.00*`, and with the localization data stored in `Bigfile_English.000`, `Bigfile_Russian.000`, etc. There are `patch*.000` files for the base game, as well, presumably for updates.
- PC Director's Cut stores everything in `BIGFILE.00*`, including localization.
...
These are found only in the base DC version, providing a list of the sections and their human-readable filenames... although the lists are too short to correspond to the entire game. I'm guessing these were for TML and were left over.

There are similar files for in the console versions (they show up under the unknown folder since the real filenames aren't known) , with the complete filenames of (seemingly) all the section data in the game.
