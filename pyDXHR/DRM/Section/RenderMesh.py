...
from pyDXHR.generated.render_model_buffer import RenderModelBuffer as KaitaiRenderModelBuffer
...
class RenderModel(RenderMesh):
    ...
    def __init__(self, section: Section):
        ...
        if buffer_mark != -1:
            self.rmb: Optional[RenderModelBuffer] = RenderModelBuffer.from_bytes(section_data[buffer_mark:])
...
class RenderTerrain(KaitaiRenderTerrain, RenderMesh):
    ...
    def parse_mesh_data(self):
        ...
        for vb, (off_vtx_buffer, unk1, len_vtx_buffer, i_fmt) in enumerate([struct.unpack_from(
                "<4L", vb_data, 0x10 * i) for i in range(header.len_vb)]):

            vtx_buffer_info = vtx_info[i_fmt]
            vtx_sem_dict = {}
            for vtx_sem in vtx_buffer_info.semantics:
                # normal2 doesn't seem to work in this specific case
                vtx_data = _read_vertex_buffer(vertex_data=self.byte_data[off_vtx_buffer:], semantic_type=vtx_sem.type,
                                               semantic_offset=vtx_sem.offset, count=len_vtx_buffer,
                                               stride=vtx_buffer_info.len_vtx, endian=self.endian)
        ...
        for i in range(header.len_nodes):
            off_list, = struct.unpack_from(f"{self.endian}L", self.byte_data, header.offset_geom + i*4)
            if off_list == 0 or off_list >= len(self.byte_data) - 2:
                continue
            n_ranges, = struct.unpack_from(f"{self.endian}H", self.byte_data, off_list)
...
class RenderModelBuffer(KaitaiRenderModelBuffer, RenderMesh):
    ...
    def parse_mesh_data(self):
        ...
        for mp in mesh_prim_list:
            ...
            mesh_prim[mp] = np.array(indices, dtype=np.uint32).reshape((mp.len_triangles, 3))
        ...
        for mesh_num, mesh in enumerate(mesh_headers):
            ...
            for i, vtx in enumerate(vtx_sem_info.semantics):
                vtx_buffer = _read_vertex_buffer(vertex_data=vtx_buffer_data, semantic_type=vtx.type,
                                                 semantic_offset=vtx.offset, count=mesh.len_vertices, stride=i_stride,
                                                 endian=self.endian)
        ...
        for grp_num, mesh_prim_list in enumerate(grouped):
            for mp in mesh_prim_list:
                ...
                mesh_prim_indexed[grp_num].append((self.materials[mp.ptr_material], mesh_prim[mp]))
...
def from_drm(drm: DRM) -> List[RenderMesh]:
    ...
    return [from_section(sec) for sec in drm.sections if sec.header.section_type == SectionType.render_mesh]
...
def _read_vertex_buffer(vertex_data: bytes,
                       semantic_type: int,
                       semantic_offset: int,
                       count: int,
                       stride: int,
    ...
    match semantic_type:
        case 0x02:
            vtx = [struct.unpack_from(
                f"{endian}fff",
                vertex_data,
                (i * stride) + semantic_offset)
