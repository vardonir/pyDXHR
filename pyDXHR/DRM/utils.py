...
def get_text_references(drm) -> List[Reference]:
    ...
    header = np.frombuffer(root_ref.section.data, dtype=np.dtype(np.uint32).newbyteorder(root_ref.endian))
    length = header[0]
    ids = header[1:]
    assert length == ids.size

    return [
        Reference.from_section_type(drm_or_section_list=drm, section_id=s_id, section_type=SectionType.dtpdata)
        for s_id in ids
    ]
